              ...
              {frameworks.map((framework) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    setSelectedFrameworks((currentSelectedFrameworks) => {
                      const isAlreadySelected = currentSelectedFrameworks.some(
                        (selectedFramework) =>
                          selectedFramework.value === currentValue,
                      );

                      if (isAlreadySelected) {
                        return currentSelectedFrameworks.filter(
                          (selectedFramework) =>
                            selectedFramework.value !== currentValue,
                        );
                      }

                      const nextFramework = frameworks.find(
                        (item) => item.value === currentValue,
                      );

                      return nextFramework
                        ? [...currentSelectedFrameworks, nextFramework]
                        : currentSelectedFrameworks;
                    });
                  }}
                  className={cn(
                    "mb-1 rounded-xl border-2 border-pink-200 bg-white px-3 py-2.5 text-pink-800 shadow-[3px_3px_0px_0px_#f472b6] transition-all outline-none hover:border-pink-400 hover:bg-pink-100 hover:text-pink-900 data-highlighted:border-pink-500 data-highlighted:bg-pink-200 data-highlighted:text-pink-950 data-highlighted:shadow-[2px_2px_0px_0px_#d62d81] data-[selected=true]:border-pink-600 data-[selected=true]:bg-pink-200 data-[selected=true]:text-pink-950 data-[selected=true]:shadow-[2px_2px_0px_0px_#d62d81]",
                  )}
                >
                  <div
                    className="pointer-events-none flex size-5 shrink-0 items-center justify-center rounded-md border-2 border-pink-300 bg-pink-50 transition-all select-none data-[selected=true]:border-pink-500 data-[selected=true]:bg-pink-400 data-[selected=true]:shadow-[2px_2px_0px_0px_#d62d81] [&_svg]:opacity-0 data-[selected=true]:[&_svg]:opacity-100"
